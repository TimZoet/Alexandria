...
void CreateTypeReferenceArray::operator()()
{
    ...
    expectNoThrow([&type0, &type1, &prop0, this]() { prop0 = &type0->createReferenceArrayProperty("prop0", *type1); });
    expectNoThrow([&type1, &type2, &type3, &prop1, &prop2, this]() {
        prop1 = &type1->createReferenceArrayProperty("prop1", *type2);
        prop2 = &type1->createReferenceArrayProperty("prop2", *type3);
    });
    expectNoThrow(
    ...
    std::vector<utils::Property> properties = {
      {0, "prop1", alex::toString(alex::DataType::NestedType), type2->getId(), 1, 1, 0},
      {0, "prop3", alex::toString(alex::DataType::Float), 0, 0, 0, 0},
      {0, "prop2", alex::toString(alex::DataType::NestedType), type3->getId(), 1, 1, 0},
      {0, "prop0", alex::toString(alex::DataType::NestedType), type1->getId(), 1, 1, 0},
    };
