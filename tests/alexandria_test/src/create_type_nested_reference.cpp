...
void CreateTypeNestedReference::operator()()
{
    ...
    expectNoThrow([&prop1, &type1, this]() { prop1 = &library->createNestedProperty("prop1", *type1, true); });
    expectNoThrow([&prop2, &type2, this]() { prop2 = &library->createNestedProperty("prop2", *type2, true); });
    expectNoThrow([&prop3, &type3, this]() { prop3 = &library->createNestedProperty("prop3", *type3, true); });
    ...
    std::vector<utils::Property> properties = {
      {0, "prop2", alex::toString(alex::DataType::NestedType), type2->getId(), 1, 0, 0},
      {0, "propA", alex::toString(alex::DataType::Float), 0, 0, 0, 0},
      {0, "prop3", alex::toString(alex::DataType::NestedType), type3->getId(), 1, 0, 0},
      {0, "prop1", alex::toString(alex::DataType::NestedType), type1->getId(), 1, 0, 0},
    };
