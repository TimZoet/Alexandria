...
void CreateTypeReferenceArray::operator()()
{
    ...
    expectThrow([&] { layout1.createReferenceArrayProperty("prop3", *type4); });
    expectNoThrow([&] {
        layout1.createReferenceArrayProperty("prop1", *type2);
        layout1.createReferenceArrayProperty("prop2", *type3);
    });
    auto [commit1, type1] = layout1.commit(*nameSpace, "type1");
    compareEQ(alex::TypeLayout::Commit::Created, commit1);

    expectNoThrow([&] { layout0.createReferenceArrayProperty("prop0", *type1); });
    ...
    const std::vector<alex::PropertyRow>  properties = {
      ...
      {4, 4, "prop1", toString(alex::DataType::Reference), 3, true, false},
      {5, 4, "prop2", toString(alex::DataType::Reference), 2, true, false},
      {6, 5, "prop0", toString(alex::DataType::Reference), 4, true, false}};
    const std::vector<alex::TableRow> tables = {{1, 2, "main_type3", "instance"},
                                                {2, 3, "main_type2", "instance"},
                                                {3, 4, "main_type1", "instance"},
                                                {4, 4, "main_type1_prop1", "reference_array"},
                                                {5, 4, "main_type1_prop2", "reference_array"},
                                                {6, 5, "main_type0", "instance"},
                                                {7, 5, "main_type0_prop0", "reference_array"}};
